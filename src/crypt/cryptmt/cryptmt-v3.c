...
#include "crypt/cryptmt/ecrypt-config.h"
#include "crypt/cryptmt/ecrypt-machine.h"
#include "crypt/cryptmt/ecrypt-portable.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* Period parameters */
#define N 156
#include "crypt/cryptmt/params.h"
struct SFMT_T {
    u32 sfmt[N][4];
};
...
static void genrand_bytes_first(cryptmt_ctx_t *ctx, u8 cipher[],
                                const u8 plain[], u32 len);
static void fast_genrand_bytes_first(cryptmt_ctx_t *ctx, u8 cipher[],
                                     const u8 plain[], u32 len);
static void genrand_bytes(cryptmt_ctx_t *ctx, u8 cipher[], const u8 plain[],
                          u32 len);
static INLINE void fast_genrand_bytes(cryptmt_ctx_t *ctx, u8 cipher[],
                                      const u8 plain[], u32 len);
static void genrand_block_first(cryptmt_ctx_t *ctx, u8 cipher[],
                                const u8 plain[]);
static void fast_genrand_block_first(cryptmt_ctx_t *ctx, u8 cipher[],
                                     const u8 plain[]);
static INLINE void genrand_block(cryptmt_ctx_t *ctx, u8 cipher[],
                                 const u8 plain[]);
static INLINE void fast_genrand_block(cryptmt_ctx_t *ctx, u8 cipher[],
                                      const u8 plain[]);
static INLINE void boot_up(cryptmt_ctx_t *ctx, s32 length);
static INLINE void fast_boot_up(cryptmt_ctx_t *ctx, s32 length);
...
static void genrand_bytes_first(cryptmt_ctx_t *ctx, u8 cipher[],
    ...
    len = len % 16;
    if (len != 0) {
        filter_bytes(sp->sfmt[i], ctx->accum, &cipher[i * 8], &plain[i * 8],
                     len);
    }
}
...
static void genrand_bytes(cryptmt_ctx_t *ctx, u8 cipher[], const u8 plain[],
    ...
    len = len % 16;
    if (len != 0) {
        filter_bytes(ps->sfmt[i], ctx->accum, &cipher[i * 8], &plain[i * 8],
                     len);
    }
}
...
static void boot_up(cryptmt_ctx_t *ctx, s32 length) {
    s32 i, p;
...
/* Key size in bits. */
/* IV size in bits. */
void ECRYPT_keysetup(cryptmt_ctx_t *ctx, const u8 *key, u32 keysize,
                     u32 ivsize) {
    s32 i;
    u32 ctx_size = sizeof(ctx);
...
void ECRYPT_ivsetup(cryptmt_ctx_t *ctx, const u8 *iv) {
...
void ECRYPT_encrypt_bytes(cryptmt_ctx_t *ctx, const u8 *plaintext,
...
/* Message length in bytes. */
void ECRYPT_decrypt_bytes(cryptmt_ctx_t *ctx, const u8 *ciphertext,
                          u8 *plaintext, u32 msglen) {
    ECRYPT_encrypt_bytes(ctx, ciphertext, plaintext, msglen);
}

/* Message length in blocks. */
void ECRYPT_encrypt_blocks(cryptmt_ctx_t *ctx, const u8 *plaintext,
...
/* Message length in blocks. */
void ECRYPT_decrypt_blocks(cryptmt_ctx_t *ctx, const u8 *ciphertext,

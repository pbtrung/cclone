...
static INLINE void _mm_genrand_block(cryptmt_ctx_t *ctx);
static INLINE void _mm_filter_16bytes(__m128i *sfmt, __m128i *p_accum,
                                      u8 cipher[], const u8 plain[], s32 count);
...
static void fast_genrand_bytes(cryptmt_ctx_t *ctx, u8 cipher[],
...
static INLINE void fast_genrand_block(cryptmt_ctx_t *ctx, u8 cipher[],
    ...
    _mm_filter_16bytes((__m128i *)ctx->psfmt, (__m128i *)ctx->accum, cipher,
                       plain, N / 2);
}
...
#endif
static void INLINE fast_genrand_bytes_first(cryptmt_ctx_t *ctx, u8 cipher[],
        const u8 plain[], u32 len) {
    s32 i, p, count;
    sfmt_t *sp;
    ...
    _mm_filter_16bytes((__m128i *)sp, (__m128i *)ctx->accum, cipher, plain,
                       len / 16);
    i = (len / 16) * 2;
    len = len % 16;
    if (len != 0) {
...
static void INLINE fast_genrand_block_first(cryptmt_ctx_t *ctx, u8 cipher[],
        const u8 plain[]) {
    s32 p;
    sfmt_t *ps;
    __m128i c;
    ...
    _mm_booter_am((__m128i *)ctx->lung, (__m128i *)&ps->sfmt[0],
                  (__m128i *)&ps->sfmt[p], N);
    _mm_filter_16bytes((__m128i *)ps, (__m128i *)ctx->accum, cipher, plain,
                       N / 2);
    ps->sfmt[0][3] = INIL;
    c = _mm_load_si128((__m128i *)ps->sfmt[N - 1]);
    c = _mm_recursion((__m128i *)ps->sfmt[0], (__m128i *)ps->sfmt[POS1], c,
                      mask);
...
static INLINE void _mm_filter_16bytes(__m128i *sfmt, __m128i *p_accum,
    ...
    mask16 = _mm_set_epi32(U32C(0x0000ffff), U32C(0x0000ffff), U32C(0x0000ffff),
                           U32C(0x0000ffff));
    mask32 = _mm_set_epi32(0, U32C(0xffffffff), 0, U32C(0xffffffff));
